...
EvaluationTemplate, EvaluationQuestion 모델 (5점 척도 문항)
TeamEvaluation/PeerEvaluation 및 응답 모델
평가 폼 화면 (제출 기능까지, 디자인은 나중)
BR-01~05 규칙을 서버 로직 또는 unique_together로 강제 (자기 팀 평가 불가, 자기 자신 평가 불가, 중복 제출 불가)
실제로 규칙을 우회 시도해보며 테스트 (본인 팀 평가 시도 → 막히는지, 같은 평가 두 번 제출 → 막히는지)
